...
import EmployeeStatusLineChart from "@/components/Dashboard/Admin/EmployeeStatusChart";
...
export default function AdminEmployeeManagement() {
  ...
  // employees status trend api call
  const { data = [], isLoading: statusLoading } = useQuery({
    queryKey: ["employeeTrend"],
    queryFn: async () => {
      const year = new Date().getFullYear();
      const res = await axiosInstance.get(
        `/admin/employee-status-trend?year=${year}`
      );
      return res.data;
    },
  });
  ...
  const firedCounts = data.map((item) => item.fired);
  ...
  if (isLoading || statusLoading) return <Spinner />;

  return (
    <div className="p-6">
      <EmployeeStatusLineChart
        months={months}
